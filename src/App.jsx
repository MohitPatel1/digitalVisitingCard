...
function Link({ link, type = '' }) {
  ...
  return (
    <a className='value' href={`${url}`} target='_blank'>
      {link}
    </a>
  );
}
...
function ContactInfo({ icon, value, type }) {
  ...
  return (
    <div className='contact-info'>
      ...
      <Link className='value' link={value} type={type}>
        {value}
      </Link>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className='flex justify-center items-center gap-1'>
        ...
        {console.log(`${website}`)}
        <Link className='value' link={website} type=''>
          {website}
        </Link>
      </div>
    </div>
  ...
  return (
    <div className='App'>
      <Routes>
        ...
        <Route path='/:companyId/:userId' element={<Users />} />
